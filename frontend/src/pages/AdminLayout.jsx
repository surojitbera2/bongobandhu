  ...
  const SidebarContent = ({ onItemClick }) => (
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-2.5 mb-8 px-1">
        <div className="w-9 h-9 rounded-xl bg-[#9333EA] flex items-center justify-center shadow-[0_8px_20px_-4px_rgba(245,158,11,0.4)]">
          <Video className="w-5 h-5 text-black" />
        </div>
        <div>
          <p className="font-heading text-base font-bold leading-none">Bongo Bandhu</p>
          <p className="text-[10px] uppercase tracking-[0.18em] text-[#9333EA] mt-1">Admin</p>
        </div>
      </div>

      <p className="text-[10px] uppercase tracking-wider text-[#64748B] font-semibold mb-2 px-1">Manage</p>
      <nav className="space-y-1 flex-1">
        {links.map((l) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all group ${
                isActive
                  ? "bg-[#9333EA]/10 text-[#9333EA] border border-[#9333EA]/25 shadow-[0_4px_20px_-8px_rgba(245,158,11,0.4)]"
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-[#9333EA] rounded-r-full" />}
      ...
      <div className="mt-4 pt-4 border-t border-white/5">
        <div className="flex items-center gap-2.5 px-1 py-2 mb-2">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#9333EA] to-[#7C3AED] flex items-center justify-center text-black text-xs font-bold">A</div>
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-[#94A3B8] hover:text-[#EF4444] hover:bg-[#EF4444]/5 border border-transparent hover:border-[#EF4444]/15 transition-all"
        >
  ...
  return (
    <div className="min-h-screen w-full bg-[#0A0D14] text-white">
      ...
      <div className="fixed inset-0 pointer-events-none -z-10">
        <div className="absolute -top-32 -left-32 w-[480px] h-[480px] rounded-full bg-[#9333EA]/[0.06] blur-[120px]" />
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full bg-[#10B981]/[0.04] blur-[120px]" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr]">
        {/* DESKTOP SIDEBAR */}
        <aside className="hidden lg:block sticky top-0 h-screen border-r border-white/5 bg-[#0D1119]/80 backdrop-blur-xl p-5">
          <SidebarContent />
        </aside>
        ...
        {drawer && (
          <>
            ...
            />
            <aside
              className="fixed top-0 left-0 bottom-0 w-72 max-w-[85vw] bg-[#0D1119] border-r border-white/10 z-50 p-5 lg:hidden shadow-2xl"
        ...
        <main className="min-w-0">
          {/* MOBILE TOPBAR */}
          <header className="lg:hidden sticky top-0 z-30 bg-[#0A0D14]/85 backdrop-blur-xl border-b border-white/5 px-4 py-3 flex items-center gap-3">
            <button
            ...
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-[#9333EA] flex items-center justify-center">
                <Video className="w-4 h-4 text-black" />
              </div>
              <div className="flex items-center gap-1.5">
                <p className="font-heading text-sm font-bold">Bongo Bandhu</p>
                <ChevronDown className="w-3 h-3 text-[#64748B]" />
                <p className="text-sm text-[#9333EA] font-semibold">{currentLabel}</p>
              </div>
            </div>
          ...
          {/* DESKTOP TOPBAR */}
          <header className="hidden lg:flex sticky top-0 z-20 bg-[#0A0D14]/85 backdrop-blur-xl border-b border-white/5 px-8 py-4 items-center">
            <p className="text-xs uppercase tracking-[0.2em] text-[#64748B]">
              <span className="text-[#9333EA]">Admin</span> · <span className="text-white">{currentLabel}</span>
            </p>
