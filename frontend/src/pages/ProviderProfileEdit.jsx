...
export default function ProviderProfileEdit() {
  ...
  const [form, setForm] = useState({ name: "", bio: "", age: 25, avatars: [], languages: [] });
  ...
  return (
    <MobileShell>
      ...
      <div className="px-5 pt-5 pb-32 space-y-5">
        {pending ? (
          <div className="p-4 rounded-2xl bg-[#9333EA]/10 border border-[#9333EA]/30 flex items-start gap-3 fade-up">
            ...
            <div>
              ...
              <p className="text-xs text-[#94A3B8] mt-1">Complete your details — admin will review and activate your listing. You won't appear in search until approved.</p>
            </div>
          </div>
        ) : (
          <div className="p-4 rounded-2xl bg-[#10B981]/10 border border-[#10B981]/30 flex items-center gap-3 fade-up">
            ...
            <div>
              ...
              <p className="text-xs text-[#94A3B8]">Your listing is visible to users.</p>
            </div>
          </div>
        )}

        <div className="bg-[#151923] border border-white/5 rounded-2xl p-5 space-y-4 fade-up delay-1">
          <div>
          ...
          <div>
            ...
            />
            <p className="text-[10px] text-[#64748B] mt-1">{form.bio.length}/300</p>
          </div>
          <div>
            <Label>Age</Label>
          ...
          <div>
            ...
            <p className="text-[10px] text-[#64748B] mt-1">Mobile is locked to your verified number.</p>
          </div>
          <div>
            <Label><Languages className="inline w-3.5 h-3.5 mr-1 -mt-0.5" /> Languages you speak</Label>
            {allLangs.length === 0 ? (
              <p className="text-xs text-[#64748B] py-2">No languages available yet. Ask admin to add some.</p>
            ) : (
                    ...
                    <button
                      ...
                      className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${
                        active
                          ...
                          : "bg-white/5 text-[#94A3B8] border-white/10 hover:border-[#9333EA]/40 hover:text-white"
                      }`}
                    >
                      {lang}
                    </button>
            ...
            <p className="text-[10px] text-[#64748B] mt-2">Tap to select / deselect. Users see your languages on your profile.</p>
          </div>
        </div>

        <div className="bg-[#151923] border border-white/5 rounded-2xl p-5 fade-up delay-2">
          <Label>Profile images <span className="text-[10px] text-[#94A3B8]">(first = cover, up to 8)</span></Label>
          ...
          <button
            ...
            className="mt-2 inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#9333EA]/40 text-[#9333EA] hover:bg-[#9333EA]/10 text-sm font-semibold disabled:opacity-50"
          >
          ...
          {form.avatars.length > 0 && (
            <div className="mt-4 grid grid-cols-3 sm:grid-cols-4 gap-2.5">
              {form.avatars.map((url, i) => (
                <div key={url + i} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 bg-[#0A0D14] group">
                  <img src={url} alt="" className="w-full h-full object-cover" />
                  {i === 0 && <span className="absolute bottom-1 left-1 text-[9px] font-bold bg-[#9333EA] text-black px-1.5 py-0.5 rounded">COVER</span>}
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent
          ...
          className="bg-[#151923] border border-white/10 text-white"
        >
          <AlertDialogHeader>
