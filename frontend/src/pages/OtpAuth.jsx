  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (r.provider && r.provider.id) {
        ...
        toast.success(r.isNew ? "Welcome to Bongo Bandhu — complete your profile" : "Welcome back");
        ...
        {step === 1 ? (
          <>
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-[#64748B] font-semibold flex items-center gap-1">
            ...
            {role === "user" ? (
              <p className="text-center text-sm text-[#94A3B8] pt-2">
                New listener?{" "}
                <button type="button" data-testid="goto-listener" onClick={() => nav("/provider/register")} className="text-[#9333EA] font-semibold hover:underline">
              ...
              <p className="text-center text-sm text-[#94A3B8] pt-2">
                Already registered?{" "}
                <button type="button" onClick={() => nav("/login")} className="text-[#9333EA] font-semibold hover:underline">
                  Sign in
                </button>
              </p>
            )}
          ...
          <>
            ...
            <div className="text-center text-sm">
              {resendIn > 0 ? (
                <span className="text-[#94A3B8]">Resend OTP in {resendIn}s</span>
              ) : (
                <button data-testid="otp-resend" onClick={sendOtp} disabled={busy} className="text-[#9333EA] font-semibold hover:underline">
                  Resend OTP
                </button>
              )}
            </div>
