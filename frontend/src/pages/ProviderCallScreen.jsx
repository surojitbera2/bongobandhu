...
export default function ProviderCallScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [provider, billing] = await Promise.all([api.getProviderMe(), api.getPublicBilling().catch(() => ({ providerSharePct: 60, packages: [] }))]);
        setMe(provider);
        setSharePct(Number(billing?.providerSharePct ?? 60));
      ...
      <div className="relative w-full h-screen overflow-hidden bg-black" onClick={tapScreen}>
        ...
        {!connected && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-20 h-20 rounded-full border-2 border-white/20 border-t-[#9333EA] spin" />
          </div>
        )}
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="font-heading text-2xl font-bold text-[#10B981] tabular-nums" data-testid="pcall-earned">{inr((computeAmount(seconds, packages) * sharePct) / 100)}</p>
            </div>
          </div>
          <div className="flex items-center justify-between text-xs text-white/70">
            <span>Package tier {inr(computeAmount(seconds, packages))}</span>
        ...
        {ended && (
          <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-xl flex items-center justify-center p-6">
            <div className="text-center">
              ...
              <p className="text-white/60 text-sm mt-2">Duration {formatDuration(seconds)} · Earned {inr((computeAmount(seconds, packages) * sharePct) / 100)}</p>
            </div>
          </div>
        )}
      </div>
