        ...
        <div className="bg-[#151923] border border-white/5 rounded-2xl p-5 fade-up">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#94A3B8]">These users cannot call you.</p>
            </div>
          </div>
        </div>

        {list.length === 0 ? (
          <div className="text-sm text-[#94A3B8] p-6 text-center bg-white/[0.02] border border-white/5 rounded-xl">
            You haven't blocked anyone yet.
          </div>
        ) : (
          <div className="space-y-2.5">
            {list.map((u) => (
              <div key={u.id} className="flex items-center justify-between p-3.5 bg-[#151923] border border-white/5 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#94A3B8]">+91 {u.mobile}</p>
                  </div>
                </div>
                <button
                  data-testid={`unblock-${u.id}`}
                  onClick={() => unblock(u.id)}
                  className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-[#9333EA]/10 border border-[#9333EA]/30 text-[#9333EA] hover:bg-[#9333EA]/20"
                >
                  Unblock
                </button>
              </div>
