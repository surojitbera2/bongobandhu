...
export default function AdminPayouts() {
  ...
  const [token] = useState(() => localStorage.getItem("navya_token") || "");
  ...
  const downloadCsv = async () => {
    try {
      ...
      a.download = `bongobandhu-payouts-${from}-to-${to}.csv`;
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#94A3B8] mt-1">Per-provider, per-call earnings — net of admin commission.</p>
        </div>
        <button
          onClick={downloadCsv}
          data-testid="payouts-csv"
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#9333EA] hover:bg-[#7C3AED] text-black font-semibold text-sm transition-colors"
        >
      ...
      {/* Filters */}
      <section className="bg-[#151923] border border-white/5 rounded-2xl p-4 sm:p-5 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">From</label>
          ...
          <div>
            <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">To</label>
            <div className="relative">
              <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#64748B] pointer-events-none" />
          ...
          <div>
            <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Provider</label>
            <div className="relative">
            ...
            {providerSearch && (
              <div className="mt-1.5 bg-[#0A0D14] border border-white/10 rounded-lg max-h-44 overflow-y-auto">
                <button onClick={() => { setProviderId(""); setProviderSearch(""); }} className="w-full text-left px-3 py-2 text-xs hover:bg-white/5 text-[#94A3B8]">All providers</button>
                {filteredProviders.slice(0, 8).map((p) => (
                  ...
                  >
                    ...
                    <span className="text-[#64748B]">+91 {p.mobile}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="flex flex-wrap gap-2">
          {presets.map((p) => (
            <button
              ...
              className="text-[11px] px-3 py-1.5 rounded-full border border-white/10 text-[#94A3B8] hover:text-white hover:border-[#9333EA]/40 transition-colors"
            >
              {p.label}
            </button>
          ))}
          {providerId && (
            <button
              ...
              className="text-[11px] px-3 py-1.5 rounded-full bg-[#9333EA]/10 text-[#9333EA] border border-[#9333EA]/30"
            >
      ...
      <section>
        <h3 className="font-heading text-sm font-semibold tracking-wide uppercase text-[#94A3B8] mb-3 flex items-center gap-2">
          ...
          <table className="w-full text-sm min-w-[860px]">
            <thead className="bg-[#0D1119] text-[#94A3B8]">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-medium text-[#9333EA]">Pending balance</th>
                <th className="px-4 py-3 text-right font-medium"></th>
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={7} className="px-4 py-8 text-center text-[#94A3B8]">Loading…</td></tr>}
              {!loading && data.summary.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-[#94A3B8]">No payouts in this range.</td></tr>
              )}
              {!loading && data.summary.map((g) => (
                <React.Fragment key={g.providerId}>
                  <tr className="border-t border-white/5 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3 text-right text-[#94A3B8] tabular-nums">{inr(g.gross)}</td>
                    <td className="px-4 py-3 text-right font-heading font-semibold text-[#10B981] tabular-nums">{inr(g.payout)}</td>
                    <td className="px-4 py-3 text-right">
                      <p className={`font-heading font-bold tabular-nums ${g.pendingBalance > 0 ? "text-[#9333EA]" : "text-[#64748B]"}`}>{inr(g.pendingBalance || 0)}</p>
                      <p className="text-[10px] text-[#64748B]">unpaid balance</p>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-md text-[#94A3B8] hover:text-white hover:bg-white/5"
                        ...
                        <button
                          ...
                          className="text-[11px] inline-flex items-center gap-1 text-[#9333EA] hover:underline"
                        >
                            ...
                            <tbody>
                              {groupRows(g.providerId).map((r) => (
                                <tr key={r.id} className="border-t border-white/5">
                                  <td className="px-2 py-2 text-[#94A3B8] whitespace-nowrap">{new Date(r.at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</td>
                                  <td className="px-2 py-2 text-white">{r.userName}<br /><span className="text-[10px] text-[#64748B]">+91 {r.userMobile}</span></td>
                                  <td className="px-2 py-2 text-right tabular-nums text-[#94A3B8]">{formatDuration(r.durationSec)}{r.autoCutoff && <span className="ml-1 text-[10px] text-[#9333EA]">·auto</span>}</td>
                                  <td className="px-2 py-2 text-right tabular-nums">{inr(r.gross)}</td>
                                  <td className="px-2 py-2 text-right tabular-nums text-[#94A3B8]">{r.bonusUsed > 0 ? inr(r.bonusUsed) : "—"}</td>
                                  <td className="px-2 py-2 text-right tabular-nums">{inr(r.realUsed)}</td>
                                  <td className="px-2 py-2 text-right tabular-nums text-[#94A3B8]">{r.sharePct}%</td>
                                  <td className="px-2 py-2 text-right tabular-nums font-semibold text-[#10B981]">{inr(r.payout)}</td>
                                  <td className="px-2 py-2 text-right tabular-nums text-[#94A3B8]">{inr(r.adminShare)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-4" data-testid="payout-dlg">
          <div className="w-full max-w-md bg-[#0F141C] border border-white/10 rounded-2xl p-6 shadow-2xl">
            <div className="flex items-start justify-between mb-4">
              ...
              <button onClick={() => setPayoutDlg(null)} className="p-1.5 -mr-2 -mt-1 rounded-lg hover:bg-white/5 text-[#94A3B8]"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Amount paid (₹)</label>
                ...
                <p className="text-[10px] text-[#64748B] mt-1">Confirms what you've already transferred to the provider's bank/UPI.</p>
              </div>
              <div>
                <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Note (UTR / bank ref — optional)</label>
              ...
              <div className="p-3 rounded-xl bg-[#9333EA]/5 border border-[#9333EA]/20">
                <p className="text-[11px] text-[#9333EA] font-semibold flex items-center gap-1.5">
                  <CheckCircle2 className="w-3 h-3" /> After confirming
                </p>
                <p className="text-[11px] text-[#94A3B8] mt-1">Provider's unpaid balance will reset to ₹0. This is recorded in payout history.</p>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setPayoutDlg(null)} className="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-[#94A3B8] text-sm font-semibold">Cancel</button>
              ...
              {historyDlg.list.length === 0 ? (
                ...
                <div className="space-y-2">
                  {historyDlg.list.map((p) => (
                    <div key={p.id} className="p-3 rounded-xl bg-[#151923] border border-white/5">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-heading font-bold text-[#10B981] tabular-nums">{inr(p.amount)}</p>
                          <p className="text-[11px] text-[#94A3B8] mt-0.5">{new Date(p.at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</p>
                        </div>
                        <CheckCircle2 className="w-5 h-5 text-[#10B981]" />
                      </div>
                      {p.note && <p className="text-[11px] text-[#94A3B8] mt-2 italic">Note: {p.note}</p>}
                      {(p.fromDate || p.toDate) && (
...
const Kpi = ({ label, value, icon: Icon, accent = false }) => (
  <div className="bg-[#151923] border border-white/5 rounded-2xl p-4 sm:p-5">
    <div className="flex items-center justify-between">
      <p className="text-[10px] uppercase tracking-wider text-[#94A3B8] font-semibold">{label}</p>
      <Icon className={`w-4 h-4 ${accent ? "text-[#9333EA]" : "text-[#64748B]"}`} />
    </div>
    <p className={`font-heading font-bold text-xl sm:text-2xl mt-2 tracking-tight ${accent ? "text-[#9333EA]" : "text-white"}`}>{value}</p>
  </div>
);
