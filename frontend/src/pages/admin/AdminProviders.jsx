    ...
    <div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-[#9333EA]/40 text-[#9333EA] hover:bg-[#9333EA]/10 text-xs font-semibold disabled:opacity-50 ${compact ? "" : ""}`}
        >
      ...
      {list.length > 0 && (
        <div className="mt-3 flex gap-2 flex-wrap">
          {list.map((url, i) => (
            <div key={url + i} className="relative group w-20 h-20 rounded-lg overflow-hidden border border-white/10 bg-[#0A0D14]">
              <img src={url} alt={`img-${i}`} className="w-full h-full object-cover" />
              {i === 0 && (
                <span className="absolute bottom-1 left-1 text-[9px] font-bold bg-[#9333EA] text-black px-1.5 py-0.5 rounded">COVER</span>
              )}
              <button
                type="button"
                onClick={() => removeAt(i)}
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="text-sm text-[#94A3B8] mt-1">Manage experts and their profiles. Call pricing is set globally in Payments.</p>
      </header>

      <section className="bg-[#151923] border border-white/5 rounded-2xl p-4 sm:p-5">
        <h3 className="font-heading text-sm font-semibold tracking-wide uppercase text-[#94A3B8] mb-4">Add provider</h3>
        ...
        <div className="mt-4">
          <p className="text-xs font-semibold tracking-wide uppercase text-[#94A3B8] mb-2 flex items-center gap-1.5"><ImageIcon className="w-3.5 h-3.5" /> Provider images</p>
          <ImageUploader value={form.avatars} onChange={(arr) => setForm({ ...form, avatars: arr })} />
        </div>
        <button data-testid="add-pr-btn" disabled={busy} onClick={add} className="mt-5 inline-flex items-center gap-2 px-5 py-2.5 bg-[#9333EA] text-white font-semibold rounded-xl hover:bg-[#7C3AED] disabled:opacity-50">
      ...
      <section className="bg-[#151923] border border-white/5 rounded-2xl overflow-x-auto">
        <table className="w-full text-sm min-w-[760px]">
          ...
          <tbody>
              ...
              <tr key={p.id} className="border-t border-white/5 align-top">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {p.avatar
                      ...
                      : <div className="w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center text-[#94A3B8]"><ImageIcon className="w-4 h-4" /></div>}
                    <div className="min-w-0">
                      <p className="text-white truncate">{p.name}</p>
                      <p className="text-[11px] text-[#94A3B8]">Age {p.age} · {(p.avatars || []).length || (p.avatar ? 1 : 0)} img · {(p.languages || []).length} lang</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-[#94A3B8]">+91 {p.mobile}</td>
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-1.5 items-start">
                    {p.status === "pending" ? (
                      <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-[#9333EA]/10 text-[#9333EA] border border-[#9333EA]/20"><Hourglass className="w-2.5 h-2.5" /> PENDING</span>
                    ) : p.status === "rejected" ? (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#EF4444]/10 text-[#EF4444] border border-[#EF4444]/20">REJECTED</span>
                    ) : (
                      <span className={`text-[10px] px-2 py-0.5 rounded-full ${p.online ? "bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/20" : "bg-white/5 text-[#94A3B8] border border-white/10"}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button data-testid={`edit-pr-${p.id}`} onClick={() => openEdit(p)} className="text-[#9333EA] p-2 rounded-lg hover:bg-[#9333EA]/10" title="Edit profile">
            ...
            {list.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-[#94A3B8]">No providers yet.</td></tr>}
          </tbody>
        </table>
      </section>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-4" data-testid="provider-edit-dlg">
          <div className="w-full max-w-2xl bg-[#0F141C] border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-start justify-between p-5 sm:p-6 border-b border-white/5">
              <div>
                <p className="text-[11px] uppercase tracking-wider text-[#9333EA] font-semibold">Edit provider</p>
                <h3 className="font-heading text-xl font-bold mt-1">{edit.name || "Untitled"}</h3>
              </div>
              <button onClick={() => setEdit(null)} className="p-1.5 -mr-1 rounded-lg hover:bg-white/5 text-[#94A3B8]"><X className="w-4 h-4" /></button>
            </div>
            <div className="overflow-y-auto p-5 sm:p-6 space-y-5">
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Name</label>
                  <Input data-testid="edit-name" value={edit.name} onChange={(e) => setEdit({ ...edit, name: e.target.value })} />
                </div>
                <div>
                  <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Mobile (10 digits)</label>
                  <Input data-testid="edit-mobile" value={edit.mobile} onChange={(e) => setEdit({ ...edit, mobile: e.target.value.replace(/\D/g, "").slice(0, 10) })} />
                </div>
              </div>
              <div>
                <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Bio</label>
                <textarea
                  data-testid="edit-bio"
                  value={edit.bio}
                  onChange={(e) => setEdit({ ...edit, bio: e.target.value })}
                  rows={3} maxLength={300}
                  className="w-full bg-[#0A0D14] border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-[#64748B] focus:outline-none focus:border-[#9333EA] text-sm"
                />
                <p className="text-[10px] text-[#64748B] mt-1">{(edit.bio || "").length}/300</p>
              </div>
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Age</label>
                  <Input data-testid="edit-age" type="number" min={18} max={99} value={edit.age} onChange={(e) => setEdit({ ...edit, age: e.target.value })} />
                </div>
                <div>
                  <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Status</label>
                  <select
                    data-testid="edit-status"
                    value={edit.status}
                    onChange={(e) => setEdit({ ...edit, status: e.target.value })}
                    className="w-full bg-[#0A0D14] border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#9333EA]"
                  >
              ...
              <div>
                <label className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5 block">Change Password (optional)</label>
                <Input 
                ...
                />
                <p className="text-[10px] text-[#64748B] mt-1">Minimum 6 characters. Only fill if you want to change.</p>
              </div>

              <div>
                <p className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-2 flex items-center gap-1.5"><LangIcon className="w-3 h-3" /> Languages</p>
                {allLangs.length === 0 ? (
                  <p className="text-xs text-[#64748B]">No languages configured. Add some in the Languages tab.</p>
                ) : (
                      ...
                      return (
                        <button
                          ...
                          className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${
                            active ? "bg-[#9333EA] text-black border-[#9333EA]" : "bg-white/5 text-[#94A3B8] border-white/10 hover:border-[#9333EA]/40"
                          }`}
                        >
                          {lang}
                        </button>
              ...
              <div>
                <p className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-3">Features</p>
                <div className="space-y-3">
                  <label className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5 cursor-pointer hover:border-[#9333EA]/30">
                    <input
                      type="checkbox"
                      checked={edit.videoCallEnabled}
                      onChange={(e) => setEdit({ ...edit, videoCallEnabled: e.target.checked })}
                      className="w-5 h-5 rounded border-white/10 bg-[#0A0D14] checked:bg-[#9333EA] checked:border-[#9333EA] focus:ring-2 focus:ring-[#9333EA]/30"
                    />
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-white">Video Call Enabled</p>
                      <p className="text-xs text-[#94A3B8]">Users can make video calls to this provider</p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5 cursor-pointer hover:border-[#9333EA]/30">
                    <input
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-[#94A3B8]">Users can request real meet via WhatsApp</p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/5 cursor-pointer hover:border-[#10B981]/30">
                    <input
                      ...
                      className="w-5 h-5 rounded border-white/10 bg-[#0A0D14] checked:bg-[#10B981] checked:border-[#10B981] focus:ring-2 focus:ring-[#10B981]/30"
                    />
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-white">Provider Online</p>
                      <p className="text-xs text-[#94A3B8]">Set provider as online/offline (users see this status)</p>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div>
                <p className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-2">Profile images</p>
            ...
            <div className="flex gap-3 p-5 sm:p-6 border-t border-white/5">
              <button onClick={() => setEdit(null)} className="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-[#94A3B8] text-sm font-semibold">Cancel</button>
              <button onClick={saveEdit} disabled={busy} data-testid="edit-save" className="flex-1 py-2.5 rounded-xl bg-[#9333EA] hover:bg-[#7C3AED] text-black text-sm font-bold inline-flex items-center justify-center gap-2 disabled:opacity-50">
