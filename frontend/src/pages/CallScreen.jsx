...
const sortedPkgs = (pkgs) =>
  (Array.isArray(pkgs) && pkgs.length ? pkgs : DEFAULT_PACKAGES)
    .map((p) => ({ minutes: Number(p.minutes) || 0, price: Number(p.price) || 0 }))
    .filter((p) => p.minutes > 0)
    .sort((a, b) => a.minutes - b.minutes);

const computeAmount = (durationSec, pkgs) => {
  if (!durationSec || durationSec <= 0) return 0;
  if (durationSec < GRACE_SEC) return 0;
  const list = sortedPkgs(pkgs);
  if (!list.length) return 0;
  const mins = durationSec / 60;
  for (const p of list) if (mins <= p.minutes) return p.price;
...
export default function CallScreen() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      // Load provider + me + billing first, store in refs
      try {
        const [p, u, billing] = await Promise.all([
          api.getProvider(providerId),
          api.getMe(),
          api.getPublicBilling().catch(() => ({ packages: DEFAULT_PACKAGES })),
        ]);
        if (!mounted) return;
        const pkgs = sortedPkgs(billing?.packages);
        setPackages(pkgs);
        packagesRef.current = pkgs;

        // Pre-call safety check: block if wallet < cheapest package
        const minPrice = pkgs[0]?.price || 0;
        if (u.wallet < minPrice) {
          toast.error(`Need at least ${inr(minPrice)} to start a call. Please recharge.`);
          nav("/wallet");
          return;
        }
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => {
      setSeconds((s) => {
        ...
        // Auto-end at the maximum package duration (e.g. 15 minutes)
        const pkgs = packagesRef.current || [];
        const maxMinutes = pkgs.length ? pkgs[pkgs.length - 1].minutes : 15;
        if (nxt >= maxMinutes * 60) {
          // schedule out of setState
  ...
  const currentAmount = computeAmount(seconds, packages);
  const nextTier = packages.find((p) => seconds / 60 <= p.minutes);
  const maxMinutes = packages.length ? packages[packages.length - 1].minutes : 15;
  const remaining = Math.max(0, maxMinutes * 60 - seconds);
      ...
      <div className="relative w-full h-screen overflow-hidden bg-black" onClick={tapScreen}>
        ...
        <div className={`absolute top-0 left-0 right-0 z-10 px-5 pt-5 transition-opacity duration-300 ${connected && !showControls ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
          <div className="flex items-center gap-2">
            <span className={`inline-block w-2 h-2 rounded-full ${connected ? "bg-[#10B981] dot-pulse" : "bg-[#9333EA] dot-pulse"}`} />
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-white/50 mt-0.5">Ends at {formatDuration(maxMinutes * 60)}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] uppercase tracking-wider text-white/60">Will charge</p>
              <p className="font-heading text-2xl font-bold text-[#9333EA] tabular-nums" data-testid="call-amount">{inr(currentAmount)}</p>
              {nextTier && currentAmount < nextTier.price && (
                <p className="text-[10px] text-white/50 mt-0.5">Next tier {inr(nextTier.price)} @ {nextTier.minutes}m</p>
              )}
            </div>
          </div>
