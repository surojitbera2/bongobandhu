    ...
    <MobileShell>
      ...
      <div className="px-5 pt-5 pb-32">
        <div className="relative overflow-hidden rounded-2xl p-6 bg-gradient-to-br from-[#1E2433] via-[#151923] to-[#0A0D14] border border-white/10 fade-up">
          ...
          <p className="text-xs uppercase tracking-[0.2em] text-[#94A3B8]">Available balance</p>
          <p className="font-heading text-5xl font-bold text-[#9333EA] mt-2 tracking-tight tabular-nums" data-testid="wallet-balance">{inr(user.wallet)}</p>
          {user.bonusBalance > 0 && (
          ...
          <p className="text-xs text-[#94A3B8] mt-2">Auto-deducts during calls.</p>
          <div className="mt-5">
        ...
        <div className="mt-7 fade-up delay-1">
          <h3 className="font-heading text-sm font-semibold tracking-wide uppercase text-[#94A3B8] mb-3">Transaction history</h3>
          {txns.length === 0 ? (
            <div className="text-sm text-[#94A3B8] p-6 text-center bg-white/[0.02] border border-white/5 rounded-xl">No transactions yet.</div>
          ) : (
            <div className="space-y-2.5">
              {txns.map((t) => (
                <div key={t.id} className="flex items-center gap-3 p-3.5 bg-[#151923] border border-white/5 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-[#94A3B8]">{timeAgo(new Date(t.at).getTime())}</p>
                  </div>
