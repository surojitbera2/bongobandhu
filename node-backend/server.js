...
const ProviderSchema = new mongoose.Schema({
  ...
  bio: String,
  age: Number,
  rate: { type: Number, default: 0 }, // DEPRECATED — billing now uses global packages
...
// ---------- Billing settings (packages + revenue split) ----------
app.get("/api/billing/public", async (_req, res) => {
  const s = await Settings.findOne({ key: "billing" });
  const v = s?.value || {};
  res.json({
    providerSharePct: Number(v.providerSharePct ?? 60),
    packages: normalizePackages(v.packages),
  });
});
...
app.put("/api/admin/billing", auth("admin"), async (req, res) => {
  const providerSharePct = Math.min(100, Math.max(0, Number(req.body?.providerSharePct ?? 60)));
  const packages = normalizePackages(req.body?.packages);
  const s = await Settings.findOneAndUpdate(
    { key: "billing" },
    { value: { providerSharePct, packages } },
    { upsert: true, new: true }
  );
  res.json(s.value);
});
...
const seedDefaults = async () => {
  ...
  if (!(await Settings.findOne({ key: "billing" }))) {
    await Settings.create({ key: "billing", value: { packages: DEFAULT_PACKAGES, providerSharePct: 60 } });
    console.log("seeded billing settings");
  } else {
    // One-time migration: if existing doc still has ratePerMin and no packages, add defaults.
    const existing = await Settings.findOne({ key: "billing" });
    const v = existing?.value || {};
    if (!Array.isArray(v.packages) || v.packages.length === 0) {
      await Settings.updateOne(
        { key: "billing" },
        { $set: { value: { providerSharePct: Number(v.providerSharePct ?? 60), packages: DEFAULT_PACKAGES } } }
      );
      console.log("migrated billing settings to packages model");
    }
  }
  ...
  socket.on("call_accept", async (msg = {}) => {
    try {
      ...
      const packages = normalizePackages(
        billing?.value?.packages
      );

      const providerSharePct = Number(
        billing?.value?.providerSharePct ?? 60
      );
